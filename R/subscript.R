...
#' @export
subscript <- function(x, ...) UseMethod('subscript')

#' @rdname subscript
#' @export
ss <- function(x, ...) subscript(x, ...)

#' @rdname subscript
#' @S3method subscript default
#' @method subscript default
subscript.default <- function(x, ...) x[...]
...
subscript.dist <- function(x, i, ...){
  ...
  # TODO: handle different kinds of subscripting (e.g. logical, character, factor)
  
  if(is.character(i)) stop('code not writen yet')
...
subscript.phylo <- function(x, i, ...){
  if(is.character(i)) stop('not writen yet')
